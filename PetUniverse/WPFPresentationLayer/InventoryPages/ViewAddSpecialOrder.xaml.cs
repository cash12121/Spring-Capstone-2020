...
namespace WPFPresentationLayer.InventoryPages
{
    ...
    public partial class ViewAddSpecialOrder : Page
    {
        ...
        private SpecialOrder _newOrder;
        private UserManager _userManager;
        private Item _item;
        private SpecialOrder _order;
        private SpecialOrderItemLine _orderItemLine;
        ...
        private void btnEditOrder_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                _newOrder = new SpecialOrder()
                {
                    SpecialOrderID = Int32.Parse(txtOrderID.Text),
                    UserID = Int32.Parse(txtUserID.Text),
                    Active = true
                };
                _orderManager.EditSpecialOrder(_order, _newOrder);
        ...
        private void btnDeleteOrderItem_Click(object sender, RoutedEventArgs e)
        {
            if (dgOrderItems.SelectedItem != null)
            {
                try
                {
                    ...
                    _orderItemLineManager.DeleteSpecialOrderItemLineByItemID(_item.ItemID);
        ...
        private void RefreshOrderItemLines()
        {
            ...
            try
            {
                List<SpecialOrderItemLine> lines = (List<SpecialOrderItemLine>)_orderItemLineManager.SelectSpecialOrderItemLinesByOrderID(_order.SpecialOrderID);
                if (lines.Count > 0)
                {
                    foreach (SpecialOrderItemLine _orderItemLine in lines)
                    {
